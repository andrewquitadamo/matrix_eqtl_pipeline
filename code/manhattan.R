...
manhattan <- function(cisResults, positions, pdf_file)
{
combined_gene_cisResults <- suppressMessages(read_delim(cisResults,"\t", escape_double = FALSE, trim_ws = TRUE))
combined_positions <- suppressMessages(read_delim(positions, "\t", escape_double = FALSE, trim_ws = TRUE))
eqtl_positions <- get_snp_positions(combined_gene_cisResults, combined_positions)
